...
function coord_errors = fixed_coord_error_func(matrix_coords, leg_params, theta_in)
    ...
    x1_fixed = x0_fixed + leg_params.crank_length.*cos(theta_in);
    y1_fixed = y0_fixed + leg_params.crank_length.*sin(theta_in);
    
    % Create error vector
    coord_errors(1) = x1_guess - x1_fixed;
    coord_errors(2) = y1_guess - y1_fixed;
